...
# Setup metrics
os.system("envsubst '$ELASTIC' < metricbeat.yaml | {kubectl} apply -f - -n elk".format(kubectl=config.kubectl_command))

# Setup logs
os.system("envsubst '$ELASTIC' < filebeat.yaml | {kubectl} apply -f - -n elk".format(kubectl=config.kubectl_command))
